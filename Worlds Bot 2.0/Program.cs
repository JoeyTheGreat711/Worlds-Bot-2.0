...
namespace Worlds_Bot_2._0
{
    class Program
    {
        ...
        ulong masterUserId = 637997156883628046;
        ulong errorDmChannel = 1100666304463126589;
        ulong selfId = 1100380213868777482;

        static int eventID = 0;// last worlds: 49726 //kalahari = 48542 open = 48473 2022 = 45258 2023 = 49725 ms = 49726
        static string url = "https://www.robotevents.com/api/v2/events/" + eventID;
        ...
        public static async Task Main()
        {
            ...
            List<int> divs = JsonConvert.DeserializeObject<Event>(await (await roboteventsClient.GetAsync(url)).Content.ReadAsStringAsync()).divisions.Select(d => d.id).Where(id => id < 100).ToList();
            foreach (int i in divs)
            {
                ...
                while (page > 0)
                {
                    ...
                    page++;
                    Division data = JsonConvert.DeserializeObject<Division>(await response.Content.ReadAsStringAsync());
        ...
        private async Task MessageRecieved(SocketMessage messageData)
        {
                ...
                try
                {
                    ...
                    if (args.Exists(s => allTeams.Exists(d => d.Contains(s))))
                    {
                        ...
                        foreach (Match m in matches)
                        {
                            ...
                            if (!String.IsNullOrEmpty(m.started) && m.name.ToLower().Contains("qualifier"))
                            {
